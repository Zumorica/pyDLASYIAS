        ...
        if self.location == "off": #Shuts down the AI
            ...
            if self.kind == "chicken":
                ...
                if self.location == "rightdoor":
                    ...
                    if self.location != "inside" or self.location != "off":
                        self.dmove("cam1a")
                return None
            ...
            #Bear's AI
            if self.kind == "bear":
                debugprnt("Bear kind is still a WIP.")
    ...
    def checkDoorTimer(self): #Timer that checks if there are animatronics at the doors
        for animatronic in animatronics:
            ...
            if animatronic.location == "leftdoor" and animatronic.kind != "fox":
                ...
                elif self.leftdoor == True: #Handles what happens if you aren't in camera mode and you have the door closed
...
#Basically the security office and the cameras.
    ...
    def securityOffice(self): #-Almost- the most important thing in main. From here you can do EVERYTHING.
        if self.power < 0 or self.time >= 6: #Checks if there's a blackout/You survived
            pass
            ...
            #Debug mode#
            usrinput = raw_input("> ")
            if usrinput.lower() in ["debug", "debugmode"]:
                global debug
                if debug == True:
                    debug = False
                else:
...
bear = animatronic("Bear", "bear", 1)

#fox, rabbit, chicken
animatronics = [rabbit, fox, chicken, bear] #Please edit this list with new animatronics if you want them to work.
