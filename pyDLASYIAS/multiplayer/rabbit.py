...
class main():
    def __init__(self, host="localhost", port=1987, fps=40, width=1280, height=720):
        ...
        self.sock.sendall(bytes("rabbit\n", "utf-8"))
        ...
        self.cooldown = random.randint(200, 450)
        ...
        while self.running:
            ...
            if self.scene == "cam":
                ...
                if spr.camButtonOneB.rect.collidepoint(Globals.pos) and Globals.mouseClick and self.cooldown == 0 and self.location != "cam1b" and not self.beingWatched and self.location in ["cam1a", "cam5"]:
                    self.changeCamera("cam1b")
                    self.cooldown = random.randint(150, 300)

                if spr.camButtonTwoA.rect.collidepoint(Globals.pos) and Globals.mouseClick and self.cooldown == 0 and self.location != "cam2a" and not self.beingWatched and self.location in ["cam1b", "cam3", "cam5"]
                    self.changeCamera("cam2a")
                    self.cooldown = random.randint(200, 500)

                if self.camButtonThree.rect.collidepoint(Globals.pos) and Globals.mouseClick and self.cooldown == 0 and self.location != "cam3" and not self.beingWatched and self.location in ["cam4a", "cam4b"]
                    self.changeCamera("cam3")
                    self.cooldown = random.randint(150, 300)

                if self.camButtonFive.rect.collidepoint(Globals.pos) and Globals.mouseClick and self.cooldown == 0 and self.location != "cam5" and not self.beingWatched and self.location in ["cam1b", "cam1a"]:
                    self.changeCamera("cam5")
                    self.cooldown = random.randint(300, 400)

                # if spr.camButtonOneB.rect.collidepoint(Globals.pos) and Globals.mouseClick and self.cooldown == 0 and self.location != "cam1b" and not self.beingWatched and self.location in ["cam1a"]:
                #     self.changeCamera("cam1b")
